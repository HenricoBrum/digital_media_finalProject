...
var available_color = Array();
available_color[0] = 0x002200;
available_color[1] = 0xFF0000;
available_color[2] = 0xCCFF66;
available_color[3] = 0x990000;
available_color[4] = 0x002600;
available_color[5] = 0x0000CC;
available_color[6] = 0x000000;
available_color[7] = 0xFFFFFF;
available_color[8] = 0x00CC33;
...
                  
      ...
      camera.position.z = (geometry.boundingBox.size().length() * 0.5)/Math.sin( Math.PI / 180.0 * camera.fov * 0.5);
...
$(document).ready(
  function ()
	...
	// Disable coloring at start.
    $('#change_color').prop('disabled', true);
    
	//Bind action to the color changing
	$('#change_color').click(
...
function ThreeDModel($scope)
        ...
        success:
          function(data)
            ...
            for(var i in $scope.threedmodels)
			  ...
			  var coloring_model_prev = available_color[ selected_color ];
  ...
  $scope.loadModel =
    function ($threedmodel, $event)
      ...
      coloring_model = available_color[ selected_color ];
      initScene("http://" + server + "/models/" + currentModel, coloring_model);
  ...
  $scope.deleteModel = 
    function ($threedmodel) 
      ...
      $.ajax(
        {
        url: "http://" + server + "/models/" + $threedmodel,
        type: "DELETE",
        processData: false,
        contentType: false,
        });
    ...
    $scope.isItSelected = function(name) {
      console.log(name);
